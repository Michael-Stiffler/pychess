...
class Board():
    ...
    def __init__(self, fen):
        ...
        self.copy_of_board = [[None for i in range(
                ...
                if isinstance(piece, Pawn):
                    if self.en_passant_possible:
                        piece.calculate_moves(
                            board, self.copy_of_en_passant_target_square)
                    else:
    ...
    def check_legal_moves(self, piece, board, pieces_on_board):
        ...
        for move in moves:
            ...
            self.get_check_moves(self.copy_of_board,
                                 self.copy_of_pieces_on_board)
    ...
    def check_if_game_over(self, move, piece, board, pieces_on_board):
        ...
        copy_of_pieces_on_board = pickle.loads(
            pickle.dumps(pieces_on_board, -1))
        self.make_move(move, piece, board=copy_of_board,
                       pieces_on_board=copy_of_pieces_on_board)
        moves = []
        ...
        for piece in copy_of_pieces_on_board:
            if piece.color == self.color_to_move:
                ...
                moves.extend(self.check_legal_moves(
                    piece, copy_of_board, copy_of_pieces_on_board))
    ...
    def get_piece_moves(self, board=None, pieces_on_board=None):
                ...
                if moves:
                    for move in moves:
                        ...
                        game_over = self.check_if_game_over(
                            move=move, piece=piece, board=board, pieces_on_board=pieces_on_board)
                        self.switch_color_to_move()
        ...
        for move in move_duplicates:
            duplicate_move_to_fixed_dict[move] = [
                x for x in fixed_move_duplicates if move[0] == x[0] and move[-1] == x[-1] and move[-2] == x[-2]]
    ...
    def check_user_move(self, start_square, end_square):
        ...
        if piece and piece.color == self.color_to_move:
            ...
            if destination is None or piece.color != destination.color:
                # if the destination square is a piece, it will be a capture
        ...
        if (self.is_enpassant and actual_move) or (self.copy_of_is_enpassant and not actual_move):
            ...
            if actual_move:
                pawn_to_remove = self.return_piece_on_square(
                    self.en_passant_destination_square, board)
            else:
                pawn_to_remove = self.return_piece_on_square(
            ...
            board[destination_square[1]][destination_square[0]
                                         ].x = destination_square[0]
            board[destination_square[1]][destination_square[0]
                                         ].y = destination_square[1]
    ...
    def make_move(self, move, piece, board=None, pieces_on_board=None):
        ...
        if isinstance(piece, Pawn):
            self.make_pawn_move(move, piece, board, pieces_on_board, start_square, destination_square,
                                piece_on_start_square, piece_on_destination_square, actual_move)
        elif isinstance(piece, King):
            self.make_king_move(move, piece, board, pieces_on_board, start_square, destination_square,
                                piece_on_start_square, piece_on_destination_square, actual_move)
        else:
            if board[destination_square[1]][destination_square[0]] is not None:
                for same_piece in pieces_on_board:
    ...
    def parse_castling_on_fen(self, castling_rights):
        kings = [self.board[y][x] for x in range(self.LENGTH) for y in range(
            self.LENGTH) if isinstance(self.board[y][x], King)]
        for king in kings:
    ...
    def promote_to_queen(self, piece, board, pieces_on_board):
        board[piece.y][piece.x] = Queen(piece.x, piece.y, self.WHITE, filename="wQ") if piece.color == self.WHITE else Queen(
            piece.x, piece.y, self.BLACK, filename="bQ")
        for same_piece in pieces_on_board:
